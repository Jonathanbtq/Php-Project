...
class Model {
    ...
    public static function GetAffectation(){
        if (Security::isConnected()){
            $user = $_SESSION['id'];
            $query = self::getInstance()->query("SELECT * FROM projets p JOIN affectation a ON p.id_projet = a.id_projet WHERE a.id_users = $user ORDER BY a.administrateur DESC");
    ...
    public static function GetTaches(){
        if (Security::isConnected()){
            $user = $_SESSION['id'];
            $query = self::getInstance()->query("SELECT *, taches.nom as 'task_name', u.nom as 'lastname' FROM taches JOIN users u ON taches.id_users = u.id_users JOIN projets p ON taches.id_projet = p.id_projet JOIN cycle_vie c ON taches.id_cycle = c.id_cycle WHERE taches.id_users = $user ORDER BY priorite DESC");
            return $query->fetchAll();
        }
    }
    ...
    public static function GetUsersList($inout){
        if (Security::isConnected()){
            ...
            if ($inout === 'in'){
                $query = self::getInstance()->query("SELECT nom, prenom, u.id_users, u.mail FROM users u JOIN affectation a ON u.id_users = a.id_users WHERE a.id_projet = $project");
            } else {
    ...
    public static function DelUserProject(){
        if(isset($_GET['deleteU'])){
            ...
            if(Security::isConnected()){
                return self::getInstance()->prepare("DELETE FROM affectation WHERE id_users = $id_user AND id_projet = $id_projet")->execute();
            }
        }
    }
    ...
    public static function DelTask($id_tache, $id_projet){
        if(Security::isConnected()){
            return self::getInstance()->prepare("DELETE FROM taches WHERE id_projet = $id_projet AND id_taches = $id_tache")->execute();
        }
    }
